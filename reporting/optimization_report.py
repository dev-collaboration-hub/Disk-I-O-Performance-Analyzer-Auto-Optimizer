"""M7 optimization planning/execution plus M8 operational alerts."""
...
from alerts.alert_engine import record_optimization_event
from alerts.alert_store import AlertStore
from config.settings import (
    ALERT_FILE, ALERT_RETENTION_RECORDS, AUTO_OPTIMIZATION_MAX_ACTIONS,
    HISTORY_FILE, OPTIMIZATION_JOURNAL_FILE,
)
...
def render_optimization_report(report: dict[str, Any]) -> str:
    ...
    for item in report.get("actions", []):
        lines.append(
            ...
            f"{item.get('target_process')} (PID {item.get('target_pid')})"
        )
    if report.get("execution_status") == "DRY_RUN":
        lines.append("No system change applied. Re-run with --apply to execute safe actions.")
    elif report.get("execution_status") == "APPLIED":
        lines.append(f"Applied {report.get('applied_count', 0)} reversible action(s); rollback is available.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plan/apply M7 optimization with M8 event logging.")
    ...
    parser.add_argument("--max-actions", type=int, default=AUTO_OPTIMIZATION_MAX_ACTIONS)
...
def main() -> None:
    ...
    if args.rollback_last:
        ...
        if not session_id or not tokens:
            report = {"status": "NO_ACTIVE_SESSION", "rolled_back_count": 0, "errors": []}
        else:
            report = rollback_tokens(
                tokens, controller=ProcessPriorityController(),
        ...
        if snapshot is None:
            report = {
                "status": "NO_DATA", "execution_status": "NO_DATA",
                "action_count": 0, "blocked_count": 0,
            }
        else:
            report = run_optimization_cycle(
                snapshot, apply=args.apply, max_actions=args.max_actions, journal=journal
            )
    try:
        record_optimization_event(
            report, store=AlertStore(args.alert_file, max_records=ALERT_RETENTION_RECORDS)
        )
    except (OSError, TypeError, ValueError) as error:
        report["alert_error"] = str(error)
